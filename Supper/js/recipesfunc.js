	...
	fetch("php/get_recipes.php")
		...
		.then((recipes) => {
			...
			recipes.forEach((recipe) => {
				...
				card.innerHTML = `
                    <img src="${recipe.image || "pics/placeholder.jpg"}" alt="${
					recipe.title
				}" width="200">
                    <div class="card-textblock">
                        ...
                        ${
							recipe.tags && recipe.tags.length > 0
								? `<div class="recipe-tags">${recipe.tags
										.map(
											(tag) =>
...
function showRecipe(id) {
	fetch(`php/get_recipe.php?id=${id}`)
		...
		.then((recipe) => {
			let ingredientsHTML = recipe.products
				.map((item) => {
			...
			let stepsHTML = recipe.steps
				.map((step, index) => {
					...
					return `
                    <div class="step-item">
                        <div class="steps-num"><p class="step-number">Шаг ${
							index + 1
						}:</p></div>
                        <img class="step-img" src="${imagePath}" alt="Шаг ${
						index + 1
					}">
                ...
                <div class="recipes-inner">
                    <div class="recipesShow">
                        ...
                        <img class="recipe-img" src="${
							recipe.image || "pics/placeholder.jpg"
						}" >
                        
                        <p class="recipe-subtitle">Наиональная кухня: ${
							recipe.subtitle
						}</p>
